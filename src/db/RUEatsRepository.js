...
module.exports = class RUEatsRepository {
  ...
  insertUser(newUser) {
    return new Promise((resolve, reject) => {
      try {
        const query =
          "INSERT INTO users (name, email, password, home_address, zip_code, city, state, verified) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        const values = [
          newUser.name,
          newUser.email,
          newUser.password,
  ...
  insertAssociate(newUser) {
    return new Promise((resolve, reject) => {
      try {
        const query =
          "INSERT INTO delivery_associates (name, email, home_address, zip_code, city, state, latitude, longitude, delivery_in_progress, password, verified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        const values = [
          newUser.name,
          newUser.email,
          newUser.home_address || null,
  ...
  getActiveOrdersByRestaurantID(restaurantID) {
    return new Promise((resolve, reject) => {
      this.connection.query(
        "SELECT * FROM orders WHERE restaurant_id = ? AND status IN (0,1)",
        [restaurantID],
        function (error, results, fields) {
          if (error) {
            reject(error);
  ...
  addMenuItemToMenu({
    ...
    return new Promise((resolve, reject) => {
      const query =
        "INSERT INTO menu (item_name, restaurant_id, description, price, spice_level, is_available, category, image_url, is_featured) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
      this.connection.query(
        query,
        [
          item_name,
  ...
  addReview(review_title, description, stars, media, author_id, restaurant_id) {
    return new Promise((resolve, reject) => {
      const date_created = new Date()
        ...
        .replace("T", " ");
      const query =
        "INSERT INTO reviews (review_title, description, stars, media, author_id, restaurant_id, date_created) VALUES (?, ?, ?, ?, ?, ?, ?)";
      this.connection.query(
        query,
        [
          review_title,
  ...
  hasUserOrderedFromRestaurant(author_id, restaurant_id) {
    return new Promise((resolve, reject) => {
      const query =
        "SELECT EXISTS(SELECT 1 FROM orders WHERE user_id = ? AND restaurant_id = ? LIMIT 1) as hasOrdered";
      this.connection.query(
        query,
  ...
  deleteMenuItem(restaurantID, itemID) {
    return new Promise((resolve, reject) => {
      this.connection.query(
        "DELETE FROM menu WHERE item_id = ? AND restaurant_id = ?",
        [itemID, restaurantID],
        function (error, results) {
          if (error) {
            reject(error);
  ...
  getInsightsByRestaurantID(restaurantID) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const orderStatusCounts = await this._getOrderStatusCounts(
          restaurantID
        );
  ...
  _getAverageRating(restaurantID) {
    return new Promise((resolve, reject) => {
      this.connection.query(
        "SELECT AVG(stars) as averageRating FROM reviews WHERE restaurant_id = ?",
        [restaurantID],
        function (error, results) {
          if (error) {
            reject(error);
  ...
  _getTotalEarnings(restaurantID) {
    return new Promise((resolve, reject) => {
      this.connection.query(
        "SELECT SUM(total_amount) as totalEarnings FROM orders WHERE restaurant_id = ?",
        [restaurantID],
        function (error, results) {
          if (error) {
            reject(error);
  ...
  async updateAssociateProfileInDB(associate_id, updates) {
    return new Promise((resolve, reject) => {
      ...
      const query = `UPDATE delivery_associates SET ${updateFields.join(
        ", "
      )} WHERE associate_id = ?`;
      this.connection.query(
        query,
        [...updateValues, associate_id],
        (error, results) => {
          if (error) {
            reject(error);
          } else {
